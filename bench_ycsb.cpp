...
DEFINE_int32(blind_write_ratio, 0, "# percents of writes are blind.");
DEFINE_int32(mirro_cache_size, 0, "# of remote keys are cached.");
...
int main(int argc, char *argv[])
{
  ...
  context.blindWriteRatio           = FLAGS_blind_write_ratio;
  ...
  context.mirro_cache_size          = FLAGS_mirro_cache_size; 

  if (FLAGS_zipf > 0) {
    context.isUniform = false;
    if (context.global_key_space) {
      aria::Zipf::globalZipf().init(context.keysPerPartition * context.partition_num, FLAGS_zipf);
    } else {
