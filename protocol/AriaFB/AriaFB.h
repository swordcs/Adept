...
public:
  ...
  void abort(TransactionType &txn, std::vector<std::unique_ptr<Message>> &messages) { txn.abort_lock = true; }
  ...
  {
    // release read locks
    calvin_release_read_locks(txn, lock_manager_id, n_lock_manager, replica_group_size);
  }
      ...
      TransactionType &txn, std::size_t lock_manager_id, std::size_t n_lock_manager, std::size_t replica_group_size)
  {
    ...
    calvin_release_read_locks(txn, lock_manager_id, n_lock_manager, replica_group_size);
    calvin_release_write_locks(txn, lock_manager_id, n_lock_manager, replica_group_size);
  ...
  void calvin_release_read_locks(
      TransactionType &txn, std::size_t lock_manager_id, std::size_t n_lock_manager, std::size_t replica_group_size)
  {
    // release read locks
  ...
  void calvin_release_write_locks(
      TransactionType &txn, std::size_t lock_manager_id, std::size_t n_lock_manager, std::size_t replica_group_size)
  {
