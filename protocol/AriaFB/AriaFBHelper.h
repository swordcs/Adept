...
public:
  ...
  static void set_key_tid(AriaFBRWKey &key, const std::tuple<std::atomic<uint64_t> *, void *> &row)
  {
    key.set_tid(std::get<0>(row));
  }

  static std::atomic<uint64_t> &get_metadata(ITable *table, const AriaFBRWKey &key)
  {
  ...
  static uint64_t get_rts(uint64_t value) { return (value >> RTS_OFFSET) & RTS_MASK; }
  ...
  static uint64_t get_wts(uint64_t value) { return (value >> WTS_OFFSET) & WTS_MASK; }
