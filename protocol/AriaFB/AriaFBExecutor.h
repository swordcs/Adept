...
public:
  ...
  AriaFBExecutor(std::size_t coordinator_id, std::size_t id, DatabaseType &db, const ContextType &context,
      std::vector<std::unique_ptr<TransactionType>> &transactions, std::vector<std::size_t> &partition_ids,
      std::vector<StorageType> &storages, std::atomic<uint32_t> &epoch, std::atomic<uint32_t> &lock_manager_status,
      ...
      : Worker(coordinator_id, id),
        ...
        partitioner(
            PartitionerFactory::create_partitioner(context.partitioner, coordinator_id, context.coordinator_num)),
        ...
        lock_manager_id(AriaFBHelper::worker_id_to_lock_manager_id(id, n_lock_manager, n_workers)),
  ...
  {
    CHECK(n_lock_manager > 0 && n_lock_manager <= 32);
    CHECK(context.worker_num > n_lock_manager);
  ...
  void start() override
  {
    ...
    for (;;) {
      ...
      // wait to AriaFB_READ
      while (static_cast<ExecutorStatus>(worker_status.load()) == ExecutorStatus::AriaFB_READ) {
        process_request();
      }
      process_request();
      n_complete_workers.fetch_add(1);
      ...
      while (static_cast<ExecutorStatus>(worker_status.load()) != ExecutorStatus::AriaFB_COMMIT) {
      ...
      while (static_cast<ExecutorStatus>(worker_status.load()) != ExecutorStatus::AriaFB_Fallback_Prepare) {
        std::this_thread::yield();
      }
      n_started_workers.fetch_add(1);
      ...
      while (static_cast<ExecutorStatus>(worker_status.load()) != ExecutorStatus::AriaFB_Fallback) {
        std::this_thread::yield();
      }
      n_started_workers.fetch_add(1);
    ...
    for (auto i = 0u; i < readSet.size(); i++) {
      ...
      if (partitioner->has_master_partition(partitionID)) {
        if (readSet[i].get_tid() == nullptr) {
          auto                   table = db.find_table(readSet[i].get_table_id(), readSet[i].get_partition_id());
          std::atomic<uint64_t> &tid   = AriaFBHelper::get_metadata(table, readSet[i]);
  ...
  void prepare_calvin_input()
  {
    ...
    for (auto i = id; i < transactions.size(); i += context.worker_num) {
      ...
      transactions[i]->setup_process_requests_in_fallback_phase(n_lock_manager, n_workers, context.coordinator_num);
    ...
    for (auto i = 0u; i < transactions.size(); i++) {
      ...
      for (auto k = 0u; k < readSet.size(); k++) {
        ...
        if (AriaFBHelper::partition_id_to_lock_manager_id(
                readKey.get_partition_id(), n_lock_manager, context.coordinator_num) != lock_manager_id) {
          continue;
        }
  ...
  void run_calvin_transactions()
  {
    ...
    while (!get_lock_manager_bit(lock_manager_id) || !transaction_queue.empty()) {
      ...
      if (result == TransactionResult::READY_TO_COMMIT) {
        ...
        protocol.calvin_commit(*transaction, lock_manager_id, n_lock_manager, context.coordinator_num);
        add_execute_time(execute_start);
        ...
        execute_start = Clock::now();
        protocol.calvin_abort(*transaction, lock_manager_id, n_lock_manager, context.coordinator_num);
        add_execute_time(execute_start);
      } else {
  ...
  void reserve_transaction(TransactionType &txn)
  {
    ...
    for (std::size_t i = 0u; i < readSet.size(); i++) {
      ...
      if (partitioner->has_master_partition(partitionId)) {
        ...
        txn.network_size += MessageFactoryType::new_reserve_message(
            *(this->messages[coordinatorID]), *table, txn.id, readKey.get_key(), txn.epoch, false);
      }
    }
    ...
    for (std::size_t i = 0u; i < writeSet.size(); i++) {
      ...
      auto         table       = db.find_table(tableId, partitionId);
      if (partitioner->has_master_partition(partitionId)) {
        std::atomic<uint64_t> &tid = AriaFBHelper::get_metadata(table, writeKey);
        writeKey.set_tid(&tid);
        AriaFBHelper::reserve_write(tid, txn.epoch, txn.id);
      } else {
        auto coordinatorID = this->partitioner->master_coordinator(partitionId);
        txn.network_size += MessageFactoryType::new_reserve_message(
            *(this->messages[coordinatorID]), *table, txn.id, writeKey.get_key(), txn.epoch, true);
      }
    }
  }
  ...
  void analyze_dependency(TransactionType &txn)
  {
    ...
    for (std::size_t i = 0u; i < readSet.size(); i++) {
      ...
      if (partitioner->has_master_partition(partitionId)) {
        ...
        txn.network_size += MessageFactoryType::new_check_message(
            *(this->messages[coordinatorID]), *table, txn.id, txn.tid_offset, readKey.get_key(), txn.epoch, false);
    ...
    for (std::size_t i = 0u; i < writeSet.size(); i++) {
      ...
      if (partitioner->has_master_partition(partitionId)) {
        ...
        auto coordinatorID = this->partitioner->master_coordinator(partitionId);
        txn.network_size += MessageFactoryType::new_check_message(
            *(this->messages[coordinatorID]), *table, txn.id, txn.tid_offset, writeKey.get_key(), txn.epoch, true);
        txn.pendingResponses++;
      }
    }
  }
  ...
  void setupHandlers(TransactionType &txn)
  {
    ...
    txn.AriaFB_read_handler = [this, &txn](AriaFBRWKey &readKey, std::size_t tid, uint32_t key_offset) {
      ...
      if (local_read || local_index_read) {
        ...
        txn.network_size += MessageFactoryType::new_search_message(
            *(this->messages[coordinatorID]), *table, tid, txn.tid_offset, key, key_offset);
        txn.distributed_transaction = true;
        txn.pendingResponses++;
      }
    };

    txn.calvin_read_handler = [this, &txn](std::size_t worker_id,
                                  std::size_t          table_id,
                                  std::size_t          partition_id,
                                  std::size_t          id,
                                  uint32_t             key_offset,
      ...
      if (worker->partitioner->has_master_partition(partition_id)) {
        ...
        for (auto i = 0u; i < active_coordinators.size(); i++) {
          if (i == worker->coordinator_id || !active_coordinators[i])
            continue;
          auto sz = MessageFactoryType::new_calvin_read_message(*worker->messages[i], *table, id, key_offset, value);
  ...
  void set_all_executors(const std::vector<AriaFBExecutor *> &executors) { all_executors = executors; }
...
private:
  ...
  std::atomic<uint32_t>                         &epoch, &lock_manager_status, &worker_status, &total_abort;
