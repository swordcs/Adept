...
{
public:
  static std::size_t new_search_message(
      Message &message, ITable &table, uint32_t tid, uint32_t tid_offset, const void *key, uint32_t key_offset)
  {
    /*
    ...
    auto message_size =
        MessagePiece::get_header_size() + key_size + sizeof(uint32_t) + sizeof(uint32_t) + sizeof(key_offset);
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(AriaFBMessage::SEARCH_REQUEST), message_size, table.tableID(), table.partitionID());
  ...
  static std::size_t new_reserve_message(
      Message &message, ITable &table, uint32_t tid, const void *key, uint32_t epoch, bool is_write)
  {
    /*
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(AriaFBMessage::RESERVE_REQUEST), message_size, table.tableID(), table.partitionID());
  ...
  static std::size_t new_check_message(Message &message, ITable &table, uint32_t tid, uint32_t tid_offset,
      const void *key, uint32_t epoch, bool is_write)
  {
    /*
    ...
    auto message_size =
        MessagePiece::get_header_size() + key_size + sizeof(uint32_t) + sizeof(uint32_t) + sizeof(epoch) + sizeof(bool);
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(AriaFBMessage::CHECK_REQUEST), message_size, table.tableID(), table.partitionID());
  ...
  static std::size_t new_write_message(Message &message, ITable &table, const void *key, const void *value)
  {
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(AriaFBMessage::WRITE_REQUEST), message_size, table.tableID(), table.partitionID());
  ...
  static std::size_t new_calvin_read_message(
      Message &message, ITable &table, uint32_t tid, uint32_t key_offset, const void *value)
  {
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(AriaFBMessage::CALVIN_READ_REQUEST), message_size, table.tableID(), table.partitionID());
...
public:
      ...
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    ...
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(AriaFBMessage::SEARCH_REQUEST));
    ...
    DCHECK(inputPiece.get_message_length() ==
           MessagePiece::get_header_size() + key_size + sizeof(tid) + sizeof(tid_offset) + sizeof(key_offset));
    ...
    auto message_size =
        MessagePiece::get_header_size() + value_size + sizeof(tid) + sizeof(tid_offset) + sizeof(key_offset);
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(AriaFBMessage::SEARCH_RESPONSE), message_size, table_id, partition_id);
      ...
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    ...
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(AriaFBMessage::SEARCH_RESPONSE));
    auto table_id     = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  static void reserve_request_handler(
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {

    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(AriaFBMessage::RESERVE_REQUEST));
    auto table_id     = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
    ...
    DCHECK(inputPiece.get_message_length() ==
           MessagePiece::get_header_size() + key_size + sizeof(tid) + sizeof(epoch) + sizeof(is_write));
  ...
  static void check_request_handler(
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {

    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(AriaFBMessage::CHECK_REQUEST));
    auto table_id     = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
    ...
    DCHECK(inputPiece.get_message_length() == MessagePiece::get_header_size() + key_size + sizeof(tid) +
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(AriaFBMessage::CHECK_RESPONSE), message_size, table_id, partition_id);
  ...
  static void check_response_handler(
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {

    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(AriaFBMessage::CHECK_RESPONSE));
    auto table_id     = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
    ...
    DCHECK(inputPiece.get_message_length() ==
           MessagePiece::get_header_size() + sizeof(tid) + sizeof(tid_offset) + 4 * sizeof(bool));
  ...
  static void write_request_handler(
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(AriaFBMessage::WRITE_REQUEST));
    auto table_id     = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  static void calvin_read_request_handler(
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(AriaFBMessage::CALVIN_READ_REQUEST));
    auto table_id     = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
    ...
    DCHECK(key_offset < txns[tid - 1]->readSet.size()) << key_offset << " " << tid;
