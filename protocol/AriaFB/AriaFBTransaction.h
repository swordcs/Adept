...
public:
  ...
  AriaFBTransaction(std::size_t coordinator_id, std::size_t partition_id, Partitioner &partitioner)
  ...
  void setup_process_requests_in_fallback_phase(
      std::size_t n_lock_manager, std::size_t n_worker, std::size_t replica_group_size)
  {
    ...
    process_requests = [this, n_lock_manager, n_worker, replica_group_size](std::size_t worker_id) {
      auto lock_manager_id = AriaFBHelper::worker_id_to_lock_manager_id(worker_id, n_lock_manager, n_worker);
...
public:
  ...
  std::function<void(AriaFBRWKey &, std::size_t, std::size_t)> AriaFB_read_handler;

  // table id, partition id, id, key_offset, key, value
  std::function<void(std::size_t, std::size_t, std::size_t, std::size_t, uint32_t, const void *, void *)>
      calvin_read_handler;
