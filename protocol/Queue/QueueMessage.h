...
public:
      ...
      Message &message, ITable &table, uint32_t tid, uint32_t key_offset, const void *value)
  {
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(QueueMessage::READ_REQUEST), message_size, table.tableID(), table.partitionID());
...
{
  using Transaction = QueueTransaction;
...
public:
  ...
  {
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(QueueMessage::READ_REQUEST));
    ...
    QueueRWKey &readKey = txns[tid]->readSet[key_offset];
