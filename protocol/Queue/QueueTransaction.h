...
#include "protocol/Queue/QueueRWKey.h"
#include <chrono>
...
public:
  ...
  QueueTransaction(std::size_t coordinator_id, std::size_t partition_id, Partitioner &partitioner)
  ...
  virtual ~QueueTransaction() = default;
  ...
  std::size_t add_to_read_set(const QueueRWKey &key)
  {
    readSet.push_back(key);
    return readSet.size() - 1;
  }

  std::size_t add_to_write_set(const QueueRWKey &key)
  {
  ...
  void build_pieces(std::size_t n_worker)
  {
    for (int i = int(readSet.size()) - 1; i >= 0; i--) {
      int32_t key = *static_cast<const int32_t *>(readSet[i].get_key());
      keyId.push_back(key % n_worker);
    }
  }
