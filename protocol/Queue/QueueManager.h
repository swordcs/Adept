...
class QueueManager : public aria::Manager
{
public:
  ...
  QueueManager(std::size_t coordinator_id, std::size_t id, DatabaseType &db, const ContextType &context,
  ...
  void add_worker(const std::shared_ptr<QueueExecutor<WorkloadType>> &w) { workers.push_back(w); }
  ...
  {
    // Queue only uses one txn generator
    this->txn_generator = static_cast<QueueTxnGenerator<WorkloadType> *>(txn_generators[0].get());
  }
