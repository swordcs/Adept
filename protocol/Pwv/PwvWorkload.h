...
public:
  ...
  PwvWorkload(DatabaseType &db, RandomType &random) { CHECK(false) << "this workload is not supported"; }
...
template <>
class PwvWorkload<ycsb::Database>
{
public:
      ...
      const ContextType &context, std::size_t partition_id, StorageType &storage)
  {
    ...
    if (context.pwv_ycsb_star) {
      auto p = std::make_unique<PwvYCSBStarTransaction>(db, context, random, storage, partition_id);
      return p;
    } else {
      auto p = std::make_unique<PwvYCSBTransaction>(db, context, random, storage, partition_id);
      return p;
    }
  }
...
template <>
class PwvWorkload<tpcc::Database>
{
public:
      ...
      const ContextType &context, std::size_t partition_id, StorageType &storage)
  {
    ...
    if (context.workloadType == tpcc::TPCCWorkloadType::MIXED) {
      if (x <= 50) {
        p = std::make_unique<PwvNewOrderTransaction>(db, context, random, storage, partition_id);
      } else {
        p = std::make_unique<PwvPaymentTransaction>(db, context, random, storage, partition_id);
      }
