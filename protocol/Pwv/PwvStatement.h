...
public:
  ...
  PwvYCSBStatement(ycsb::Database &db, const ycsb::Context &context, ycsb::Random &random, ycsb::Storage &storage,
      std::size_t partition_id, const ycsb::YCSBQuery<keys_num> &query, int idx)
      : db(db), context(context), random(random), storage(storage), partition_id(partition_id), query(query), idx(idx)
  {}
  ...
  void prepare_read_and_write_set() override
  {
    ...
    CHECK(query.UPDATE[idx] ? writeSet.size() == 1 : writeSet.size() == 0) << "Check on writeSet size failed!";
  }
  ...
  void execute() override
  {
    ...
    if (query.UPDATE[idx]) {
      ...
      storage.ycsb_values[idx].Y_F01.assign(local_random.a_string(ycsb::YCSB_FIELD_SIZE, ycsb::YCSB_FIELD_SIZE));
      storage.ycsb_values[idx].Y_F02.assign(local_random.a_string(ycsb::YCSB_FIELD_SIZE, ycsb::YCSB_FIELD_SIZE));
      storage.ycsb_values[idx].Y_F03.assign(local_random.a_string(ycsb::YCSB_FIELD_SIZE, ycsb::YCSB_FIELD_SIZE));
      storage.ycsb_values[idx].Y_F04.assign(local_random.a_string(ycsb::YCSB_FIELD_SIZE, ycsb::YCSB_FIELD_SIZE));
      storage.ycsb_values[idx].Y_F05.assign(local_random.a_string(ycsb::YCSB_FIELD_SIZE, ycsb::YCSB_FIELD_SIZE));
      storage.ycsb_values[idx].Y_F06.assign(local_random.a_string(ycsb::YCSB_FIELD_SIZE, ycsb::YCSB_FIELD_SIZE));
      storage.ycsb_values[idx].Y_F07.assign(local_random.a_string(ycsb::YCSB_FIELD_SIZE, ycsb::YCSB_FIELD_SIZE));
      storage.ycsb_values[idx].Y_F08.assign(local_random.a_string(ycsb::YCSB_FIELD_SIZE, ycsb::YCSB_FIELD_SIZE));
      storage.ycsb_values[idx].Y_F09.assign(local_random.a_string(ycsb::YCSB_FIELD_SIZE, ycsb::YCSB_FIELD_SIZE));
      storage.ycsb_values[idx].Y_F10.assign(local_random.a_string(ycsb::YCSB_FIELD_SIZE, ycsb::YCSB_FIELD_SIZE));
    }
...
public:
  ...
  PwvYCSBStarStatement(ycsb::Database &db, const ycsb::Context &context, ycsb::Random &random, ycsb::Storage &storage,
      std::size_t partition_id, const ycsb::YCSBQuery<keys_num> &query, int idx, bool read,
...
{
public:
  PwvNewOrderWarehouseStatement(tpcc::Database &db, const tpcc::Context &context, tpcc::Random &random,
      tpcc::Storage &storage, std::size_t partition_id, const tpcc::NewOrderQuery &query)
      : db(db), context(context), random(random), storage(storage), partition_id(partition_id), query(query)
  {}
  ...
  void execute() override
  {
    auto *warehouse_table = db.find_table(tpcc::warehouse::tableID, partition_id);
    PwvHelper::read(warehouse_table->search(&storage.warehouse_key),
        &storage.warehouse_value,
        warehouse_table->value_size());

    auto *district_table = db.find_table(tpcc::district::tableID, partition_id);
    PwvHelper::read(
        district_table->search(&storage.district_key), &storage.district_value, district_table->value_size());

    auto *customer_table = db.find_table(tpcc::customer::tableID, partition_id);
    PwvHelper::read(
        customer_table->search(&storage.customer_key), &storage.customer_value, customer_table->value_size());

    storage.district_value.D_NEXT_O_ID += 1;
    district_table->update(&storage.district_key, &storage.district_value);
  }
...
{
public:
  PwvNewOrderStockStatement(tpcc::Database &db, const tpcc::Context &context, tpcc::Random &random,
      tpcc::Storage &storage, std::size_t partition_id, const tpcc::NewOrderQuery &query, int idx,
      std::atomic<int> &commit_rvp)
      : db(db),
        context(context),
        random(random),
  ...
  void execute() override
  {
    ...
    if (storage.stock_values[idx].S_QUANTITY >= OL_QUANTITY + 10) {
      ...
      storage.stock_values[idx].S_QUANTITY = storage.stock_values[idx].S_QUANTITY - OL_QUANTITY + 91;
    }
...
{
public:
  PwvNewOrderOrderStatement(tpcc::Database &db, const tpcc::Context &context, tpcc::Random &random,
      tpcc::Storage &storage, std::size_t partition_id, const tpcc::NewOrderQuery &query, float &total_amount)
      : db(db),
        context(context),
        random(random),
        storage(storage),
  ...
  void execute() override
  {
    ...
    for (int i = 0; i < query.O_OL_CNT; i++) {
      ...
      storage.order_line_keys[i] = tpcc::order_line::key(W_ID, D_ID, D_NEXT_O_ID, i + 1);
      ...
      switch (D_ID) {
        case 1: storage.order_line_values[i].OL_DIST_INFO = storage.stock_values[i].S_DIST_01; break;
        case 2: storage.order_line_values[i].OL_DIST_INFO = storage.stock_values[i].S_DIST_02; break;
        case 3: storage.order_line_values[i].OL_DIST_INFO = storage.stock_values[i].S_DIST_03; break;
        case 4: storage.order_line_values[i].OL_DIST_INFO = storage.stock_values[i].S_DIST_04; break;
        case 5: storage.order_line_values[i].OL_DIST_INFO = storage.stock_values[i].S_DIST_05; break;
        case 6: storage.order_line_values[i].OL_DIST_INFO = storage.stock_values[i].S_DIST_06; break;
        case 7: storage.order_line_values[i].OL_DIST_INFO = storage.stock_values[i].S_DIST_07; break;
        case 8: storage.order_line_values[i].OL_DIST_INFO = storage.stock_values[i].S_DIST_08; break;
        case 9: storage.order_line_values[i].OL_DIST_INFO = storage.stock_values[i].S_DIST_09; break;
        case 10: storage.order_line_values[i].OL_DIST_INFO = storage.stock_values[i].S_DIST_10; break;
...
{
public:
  PwvPaymentDistrictStatement(tpcc::Database &db, const tpcc::Context &context, tpcc::Random &random,
      tpcc::Storage &storage, std::size_t partition_id, const tpcc::PaymentQuery &query)
      : db(db), context(context), random(random), storage(storage), partition_id(partition_id), query(query)
  {}
  ...
  void prepare_read_and_write_set() override
  {
    ...
    CHECK(readSet.size() == (context.write_to_w_ytd ? 2 : 1)) << "Check on readSet size failed!";
    CHECK(writeSet.size() == (context.write_to_w_ytd ? 2 : 1)) << "Check on writeSet size failed!";
  }
...
{
public:
  PwvPaymentCustomerStatement(tpcc::Database &db, const tpcc::Context &context, tpcc::Random &random,
      tpcc::Storage &storage, std::size_t partition_id, const tpcc::PaymentQuery &query)
      : db(db), context(context), random(random), storage(storage), partition_id(partition_id), query(query)
  {}
  ...
  void execute() override
  {
    ...
    written =
        std::sprintf(H_DATA, "%s    %s", storage.warehouse_value.W_NAME.c_str(), storage.district_value.D_NAME.c_str());
    H_DATA[written] = 0;

    storage.h_key            = tpcc::history::key(W_ID, D_ID, C_W_ID, C_D_ID, C_ID, Time::now());
