...
class CaracalManager : public aria::Manager
{
public:
  ...
  CaracalManager(std::size_t coordinator_id, std::size_t id, DatabaseType &db, const ContextType &context,
      std::atomic<bool> &stopFlag)
      : base_type(coordinator_id, id, context, stopFlag),
        db(db),
        epoch(0),
        partitioner(coordinator_id, context.coordinator_num),
        piece_scheduler(context.worker_num)
  {
  ...
  void coordinator_start() override
  {
    ...
    while (!stopFlag.load()) {
      ...
      contention.analyze(db, transactions, context.caracal_threshold);
      ...
      n_started_workers.store(0);
      n_completed_workers.store(0);
      signal_worker(ExecutorStatus::Caracal_Insert);
      wait_all_workers_start();
      wait_all_workers_finish();
      ...
      n_started_workers.store(0);
      n_completed_workers.store(0);
      signal_worker(ExecutorStatus::Caracal_Execute);
      wait_all_workers_start();
      wait_all_workers_finish();
      // wait for all machines until they finish the execution phase.
      wait4_ack();
  ...
  void non_coordinator_start() override
  {
    ...
    for (;;) {
      ...
      status = wait4_signal();
      DCHECK(status == ExecutorStatus::Caracal_Insert);
      // Allow each worker to run transactions
      n_started_workers.store(0);
      n_completed_workers.store(0);
      set_worker_status(ExecutorStatus::Caracal_Insert);
      wait_all_workers_start();
      wait_all_workers_finish();
      send_ack();

      status = wait4_signal();
      DCHECK(status == ExecutorStatus::Caracal_Execute);
      // Allow each worker to run transactions
      n_started_workers.store(0);
      n_completed_workers.store(0);
      set_worker_status(ExecutorStatus::Caracal_Execute);
      wait_all_workers_start();
      wait_all_workers_finish();
      send_ack();
