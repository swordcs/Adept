...
public:
  ...
  static void read(const std::tuple<MetaDataType *, void *> &row, void *dest, std::size_t size)
  {
    ...
    DCHECK(is_placeholder_ready(placeholder)) << "placeholder not ready to read.";
    void *src = std::get<1>(row);
    std::memcpy(dest, src, size);
  }

  // value 0 meaning the value is ready to read
  static bool is_placeholder_ready(std::atomic<uint64_t> &a) { return a.load(std::memory_order_acquire) == 0; }

  static void set_placeholder_to_ready(std::atomic<uint64_t> &a) { a.store(0, std::memory_order_release); }
  ...
  static std::size_t partition_id_to_worker_id(
      std::size_t partition_id, std::size_t n_worker, std::size_t n_coordinator)
  {
    return partition_id / n_coordinator % n_worker;
  }

public:
  static uint64_t get_epoch(uint64_t value) { return (value >> EPOCH_OFFSET) & EPOCH_MASK; }
  ...
  static uint64_t get_pos(uint64_t value) { return (value >> POS_OFFSET) & POS_MASK; }
