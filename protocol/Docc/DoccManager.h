...
class DoccManager : public aria::Manager
{
public:
  ...
  DoccManager(std::size_t coordinator_id, std::size_t id, DatabaseType &db, const ContextType &context,
  ...
  void coordinator_start() override
  {
    ...
    while (!stopFlag.load()) {
      ...
      signal_worker(ExecutorStatus::Docc_READ);
      ...
      wait_all_workers_finish();
      // wait for all machines until they finish the Docc_READ phase.
      wait4_ack();
      ...
      set_worker_status(ExecutorStatus::STOP);
      wait_all_workers_finish();
      // wait for all machines until they finish the Docc_COMMIT phase.
      wait4_ack();
    }
  ...
  void non_coordinator_start() override
  {
    ...
    for (;;) {
      ...
      DCHECK(status == ExecutorStatus::Docc_READ);
      // the coordinator on each machine first moves the aborted transactions
      // from the last batch earlier to the next batch and set remaining
      // transaction slots to null.
      ...
      n_started_workers.store(0);
      n_completed_workers.store(0);
      set_worker_status(ExecutorStatus::Docc_READ);
      wait_all_workers_start();
      wait_all_workers_finish();
      broadcast_stop();
      wait4_stop(n_coordinators - 1);
