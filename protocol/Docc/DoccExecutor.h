...
class DoccExecutor : public Worker
{
public:
  ...
  DoccExecutor(std::size_t coordinator_id, std::size_t id, DatabaseType &db, const ContextType &context,
      std::vector<std::unique_ptr<TransactionType>> &transactions, std::vector<StorageType> &storages,
      std::atomic<uint32_t> &epoch, std::atomic<uint32_t> &worker_status, std::atomic<uint32_t> &total_abort,
  ...
  void start() override
  {
    ...
    for (;;) {
      ...
      } while (status != ExecutorStatus::Docc_READ);

      n_started_workers.fetch_add(1);
      read_snapshot();
      n_complete_workers.fetch_add(1);
      // wait to Docc_READ
      while (static_cast<ExecutorStatus>(worker_status.load()) == ExecutorStatus::Docc_READ) {
        process_request();
      }
      process_request();
      n_complete_workers.fetch_add(1);

      // wait till Docc_COMMIT
      while (static_cast<ExecutorStatus>(worker_status.load()) != ExecutorStatus::Docc_COMMIT) {
  ...
  void read_snapshot_single(TransactionType &txn)
  {
    if (txn.distributed_transaction) {
      simulate_distribute_retry_cost();
    }
  }
  ...
  void reserve_transaction(TransactionType &txn)
  {
    ...
    std::vector<DoccRWKey> &readSet  = txn.readSet;
    std::vector<DoccRWKey> &writeSet = txn.writeSet;
    ...
    for (std::size_t i = 0u; i < readSet.size(); i++) {
      ...
      if (partitioner->has_master_partition(partitionId)) {
        std::atomic<uint64_t> &tid = DoccHelper::get_metadata(table, readKey);
        readKey.set_tid(&tid);
        DoccHelper::reserve_read(tid, txn.epoch, txn.id);
      } else {
    ...
    for (std::size_t i = 0u; i < writeSet.size(); i++) {
      DoccRWKey &writeKey    = writeSet[i];
      auto       tableId     = writeKey.get_table_id();
      auto       partitionId = writeKey.get_partition_id();
      auto       table       = db.find_table(tableId, partitionId);
      if (partitioner->has_master_partition(partitionId)) {
        std::atomic<uint64_t> &tid = DoccHelper::get_metadata(table, writeKey);
        writeKey.set_tid(&tid);
        DoccHelper::reserve_write(tid, txn.epoch, txn.id);
      } else {
        auto coordinatorID = this->partitioner->master_coordinator(partitionId);
        txn.network_size += MessageFactoryType::new_reserve_message(
  ...
  void analyze_dependency(TransactionType &txn)
  {
    ...
    const std::vector<DoccRWKey> &readSet  = txn.readSet;
    const std::vector<DoccRWKey> &writeSet = txn.writeSet;

    // analyze raw

    for (std::size_t i = 0u; i < readSet.size(); i++) {
      const DoccRWKey &readKey = readSet[i];
      if (readKey.get_local_index_read_bit())
        continue;
      ...
      if (partitioner->has_master_partition(partitionId)) {
        uint64_t tid   = DoccHelper::get_metadata(table, readKey).load();
        uint64_t epoch = DoccHelper::get_epoch(tid);
        uint64_t wts   = DoccHelper::get_wts(tid);
    ...
    for (std::size_t i = 0u; i < writeSet.size(); i++) {
      const DoccRWKey &writeKey = writeSet[i];
      ...
      if (partitioner->has_master_partition(partitionId)) {
        uint64_t tid   = DoccHelper::get_metadata(table, writeKey).load();
        uint64_t epoch = DoccHelper::get_epoch(tid);
        uint64_t rts   = DoccHelper::get_rts(tid);
        uint64_t wts   = DoccHelper::get_wts(tid);
        DCHECK(epoch == txn.epoch);
  ...
  {
    std::size_t count = 0;
    for (auto i = id; i < transactions.size(); i += 1) {
      if (transactions[i]->abort_no_retry)
        continue;
    ...
    for (auto i = id; i < transactions.size(); i += 1) {
      ...
      if (transactions[i]->raw || transactions[i]->waw) {
  ...
  void setupHandlers(TransactionType &txn)
  {
    ...
    txn.readRequestHandler = [this, &txn](DoccRWKey &readKey, std::size_t tid, uint32_t key_offset) {
      ...
      if (local_read || local_index_read) {
        ...
        DoccHelper::set_key_tid(readKey, row);
        DoccHelper::read(row, value, table->value_size());
      } else {
