...
public:
      ...
      Message &message, ITable &table, uint32_t tid, uint32_t tid_offset, const void *key, uint32_t key_offset)
  {
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(DoccMessage::SEARCH_REQUEST), message_size, table.tableID(), table.partitionID());
      ...
      Message &message, ITable &table, uint32_t tid, const void *key, uint32_t epoch, bool is_write)
  {
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(DoccMessage::RESERVE_REQUEST), message_size, table.tableID(), table.partitionID());
      ...
      const void *key, uint32_t epoch, bool is_write)
  {
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(DoccMessage::CHECK_REQUEST), message_size, table.tableID(), table.partitionID());
  ...
  static std::size_t new_write_message(Message &message, ITable &table, const void *key, const void *value)
  {
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(DoccMessage::WRITE_REQUEST), message_size, table.tableID(), table.partitionID());
...
{
  using Transaction = DoccTransaction;
...
public:
      ...
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    ...
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(DoccMessage::SEARCH_REQUEST));
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(DoccMessage::SEARCH_RESPONSE), message_size, table_id, partition_id);
    ...
    DoccHelper::read(row, dest, value_size);
    encoder << tid << tid_offset << key_offset;
      ...
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    ...
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(DoccMessage::SEARCH_RESPONSE));
    auto table_id     = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
      ...
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    ...
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(DoccMessage::RESERVE_REQUEST));
    auto table_id     = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
    ...
    if (is_write) {
      DoccHelper::reserve_write(metadata, epoch, tid);
    } else {
      DoccHelper::reserve_read(metadata, epoch, tid);
    }
  }
      ...
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    ...
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(DoccMessage::CHECK_REQUEST));
    auto table_id     = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
    ...
    if (is_write) {
      ...
      // analyze war and waw
      uint64_t reserve_epoch = DoccHelper::get_epoch(metadata);
      uint64_t reserve_rts   = DoccHelper::get_rts(metadata);
      uint64_t reserve_wts   = DoccHelper::get_wts(metadata);
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(DoccMessage::CHECK_RESPONSE), message_size, table_id, partition_id);
      ...
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    ...
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(DoccMessage::CHECK_RESPONSE));
    auto table_id     = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  static void write_request_handler(
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(DoccMessage::WRITE_REQUEST));
    auto table_id     = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
