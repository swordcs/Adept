...
class DoccTransaction
{

public:
  using MetaDataType = std::atomic<uint64_t>;

  DoccTransaction(std::size_t coordinator_id, std::size_t partition_id, Partitioner &partitioner)
  ...
  virtual ~DoccTransaction() = default;
...
public:
  ...
  std::function<void(DoccRWKey &, std::size_t, std::size_t)> readRequestHandler;
  ...
  std::vector<DoccRWKey> readSet, writeSet;
};
}  // namespace aria