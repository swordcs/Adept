...
public:
  ...
  static void set_key_tid(DoccRWKey &key, const std::tuple<std::atomic<uint64_t> *, void *> &row)
  {
    key.set_tid(std::get<0>(row));
  }

  static std::atomic<uint64_t> &get_metadata(ITable *table, const DoccRWKey &key)
  {
