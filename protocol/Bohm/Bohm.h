...
public:
  ...
  Bohm(DatabaseType &db, BohmPartitioner &partitioner) : db(db), partitioner(partitioner) {}
  ...
  void commit(TransactionType &txn, std::vector<std::unique_ptr<Message>> &messages)
  {
    ...
    for (auto i = 0u; i < writeSet.size(); i++) {
      ...
      if (partitioner.has_master_partition(partitionId)) {
        std::atomic<uint64_t> &placeholder = table->search_metadata(key, txn.id);
        ...
        txn.network_size += MessageFactoryType::new_write_message(*messages[coordinatorID], *table, txn.id, key, value);
