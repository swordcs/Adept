...
public:
      ...
      Message &message, ITable &table, uint64_t tid, uint32_t key_offset, const void *key)
  {
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(BohmMessage::READ_REQUEST), message_size, table.tableID(), table.partitionID());
      ...
      Message &message, ITable &table, uint64_t tid, const void *key, const void *value)
  {
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(BohmMessage::WRITE_REQUEST), message_size, table.tableID(), table.partitionID());
  ...
  static std::size_t new_write_response_message(Message &message, ITable &table, uint64_t tid)
  {
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(BohmMessage::WRITE_RESPONSE), message_size, table.tableID(), table.partitionID());
...
public:
      ...
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    ...
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(BohmMessage::READ_REQUEST));
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(BohmMessage::READ_RESPONSE), message_size, table_id, partition_id);
      ...
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    ...
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(BohmMessage::READ_RESPONSE));
    auto table_id     = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  static void write_request_handler(
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(BohmMessage::WRITE_REQUEST));
    auto table_id     = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
    ...
    BohmMessageFactory::new_write_response_message(responseMessage, table, tid);
  }

  static void write_response_handler(
      MessagePiece inputPiece, Message &responseMessage, ITable &table, std::vector<std::unique_ptr<Transaction>> &txns)
  {
    DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(BohmMessage::WRITE_RESPONSE));
