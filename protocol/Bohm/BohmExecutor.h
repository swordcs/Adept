...
class BohmExecutor : public Worker
{
public:
  ...
  BohmExecutor(std::size_t coordinator_id, std::size_t id, DatabaseType &db, const ContextType &context,
  ...
  void start() override
  {
    ...
    for (;;) {
      ...
      // wait to Execute
      while (static_cast<ExecutorStatus>(worker_status.load()) == ExecutorStatus::Bohm_Analysis) {
      ...
      n_complete_workers.fetch_add(1);
      // wait to insert
      while (static_cast<ExecutorStatus>(worker_status.load()) == ExecutorStatus::Bohm_Insert) {
        process_request();
      }
      ...
      n_complete_workers.fetch_add(1);
      // wait to execute
      while (static_cast<ExecutorStatus>(worker_status.load()) == ExecutorStatus::Bohm_Execute) {
        process_request();
      }
      ...
      n_complete_workers.fetch_add(1);
      // wait to gc
      while (static_cast<ExecutorStatus>(worker_status.load()) == ExecutorStatus::Bohm_GC) {
        process_request();
      }
    }
  }
  ...
  {
    txn.read_handler = [this, &txn](BohmRWKey &readKey, std::size_t tid, uint32_t key_offset) {
    ...
    for (auto i = 0u; i < transactions.size(); i++) {
      ...
      for (auto k = 0u; k < writeSet.size(); k++) {
        ...
        if (partitioner.has_master_partition(partitionId) &&
            BohmHelper::partition_id_to_worker_id(partitionId, context.worker_num, context.coordinator_num) == id) {
    ...
    auto run_transaction = [this](TransactionType *transaction) {
      ...
      for (;;) {
        ...
        if (result == TransactionResult::READY_TO_COMMIT) {
          ...
          flush_messages();
          while (transaction->pendingResponses > 0) {
            process_request();
          }
          ...
          if (context.sleep_on_retry) {
            std::this_thread::sleep_for(std::chrono::microseconds(sleep_random.uniform_dist(0, context.sleep_time)));
          }
        } else {
