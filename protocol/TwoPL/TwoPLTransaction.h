...
public:
  ...
  TwoPLTransaction(std::size_t coordinator_id, std::size_t partition_id, Partitioner &partitioner)
  ...
  bool process_requests(std::size_t worker_id)
  {
    ...
    if (pendingResponses > 0) {
      ...
      if (network_wait_handler) {
        network_wait_handler(wait_start);
      }
    }
    return false;
  }
...
public:
  ...
  // success?, remote?
  std::function<uint64_t(std::size_t, std::size_t, uint32_t, const void *, void *, bool, bool, bool &, bool &)>
      lock_request_handler;
  // processed a request?
  std::function<std::size_t(void)> remote_request_handler;

  std::function<void()> message_flusher;
  std::function<void(std::chrono::steady_clock::time_point)> network_wait_handler;
