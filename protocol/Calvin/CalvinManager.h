...
public:
  ...
  CalvinManager(std::size_t coordinator_id, std::size_t id, DatabaseType &db, const ContextType &context,
  ...
  {
    // Calvin only uses one txn generator
    this->txn_generator = static_cast<CalvinTxnGenerator<WorkloadType> *>(txn_generators[0].get());
  }
...
public:
  ...
  std::atomic<uint32_t>             epoch;  // add epoch to CalvinManager
  CalvinTxnGenerator<WorkloadType> *txn_generator;
};
}  // namespace aria