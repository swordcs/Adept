...
private:
  void make_multi_partitions(YCSBQuery<N> &query, const Context &context, uint32_t partitionID, Random &random) const
  {
    ...
    for (auto i = 0u; i < N; i++) {
      ...
      if (readOnly <= context.readOnlyTransaction) {
        ...
        if (readOrWrite <= context.readWriteRatio) {
          ...
          query.UPDATE[i] = true;
          int blind       = random.uniform_dist(1, 100);
          if (blind <= context.blindWriteRatio) {
            query.BLIND[i] = true;
          }
        }
      }
      ...
      do {
        ...
        if (context.isUniform || (context.skewPattern == YCSBSkewPattern::READ && query.UPDATE[i]) ||
            (context.skewPattern == YCSBSkewPattern::WRITE && query.UPDATE[i] == false)) {
  ...
  void make_two_partitions(YCSBQuery<N> &query, const Context &context, uint32_t partitionID, Random &random) const
  {
    int  readOnly       = random.uniform_dist(1, 100);
    int  crossPartition = random.uniform_dist(1, 100);
    auto newPartitionID = partitionID;
  ...
  void make_global_key_space_query(
      YCSBQuery<N> &query, const Context &context, uint32_t partitionID, Random &random) const
  {
    int readOnly = random.uniform_dist(1, 100);
