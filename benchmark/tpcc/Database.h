...
public:
  ...
  void initTables(const std::string &name, InitFunc initFunc, std::size_t partitionNum, std::size_t threadsNum,
  ...
  void initialize(const Context &context)
  {
    ...
    for (auto partitionID = 0u; partitionID < partitionNum; partitionID++) {
      ...
      tbl_warehouse_vec.push_back(
          TableFactory::create_table<997, warehouse::key, warehouse::value>(context, warehouseTableID, partitionID));

      auto districtTableID = district::tableID;
      tbl_district_vec.push_back(
          TableFactory::create_table<997, district::key, district::value>(context, districtTableID, partitionID));

      auto customerTableID = customer::tableID;
      tbl_customer_vec.push_back(
          TableFactory::create_table<997, customer::key, customer::value>(context, customerTableID, partitionID));

      auto customerNameIdxTableID = customer_name_idx::tableID;
      tbl_customer_name_idx_vec.push_back(
          TableFactory::create_table<997, customer_name_idx::key, customer_name_idx::value>(
              context, customerNameIdxTableID, partitionID));

      auto historyTableID = history::tableID;
      tbl_history_vec.push_back(
          TableFactory::create_table<997, history::key, history::value>(context, historyTableID, partitionID));

      auto newOrderTableID = new_order::tableID;
      tbl_new_order_vec.push_back(
          TableFactory::create_table<997, new_order::key, new_order::value>(context, newOrderTableID, partitionID));

      auto orderTableID = order::tableID;
      tbl_order_vec.push_back(
          TableFactory::create_table<997, order::key, order::value>(context, orderTableID, partitionID));

      auto orderLineTableID = order_line::tableID;
      tbl_order_line_vec.push_back(
          TableFactory::create_table<997, order_line::key, order_line::value>(context, orderLineTableID, partitionID));

      auto stockTableID = stock::tableID;
      tbl_stock_vec.push_back(
          TableFactory::create_table<997, stock::key, stock::value>(context, stockTableID, partitionID));
    }
    ...
    std::transform(
        tbl_customer_name_idx_vec.begin(), tbl_customer_name_idx_vec.end(), std::back_inserter(tbl_vecs[3]), tFunc);
    ...
    initTables(
        "warehouse",
        [&context, this](std::size_t partitionID) { warehouseInit(context, partitionID); },
        partitionNum,
        threadsNum,
        partitioner.get());
    initTables(
        "district",
        [&context, this](std::size_t partitionID) { districtInit(context, partitionID); },
        partitionNum,
        threadsNum,
        partitioner.get());
    initTables(
        "customer",
        [&context, this](std::size_t partitionID) { customerInit(context, partitionID); },
        partitionNum,
        threadsNum,
        partitioner.get());
    initTables(
        "customer_name_idx",
        [&context, this](std::size_t partitionID) { customerNameIdxInit(context, partitionID); },
        partitionNum,
        threadsNum,
        partitioner.get());
    /*    initTables("history",
        ...
        initTables("order_line",
                   ...
                   partitionNum, threadsNum, partitioner.get());*/
    initTables(
        "item", [&context, this](std::size_t partitionID) { itemInit(context, partitionID); }, 1, 1, nullptr);
    initTables(
        "stock",
        [&context, this](std::size_t partitionID) { stockInit(context, partitionID); },
        partitionNum,
        threadsNum,
        partitioner.get());
  }
...
private:
  ...
  void customerNameIdxInit(const Context &context, std::size_t partitionID)
  {
    ...
    std::unordered_map<FixedString<16>, std::vector<std::tuple<FixedString<16>, int32_t>>>
    ...
    for (int i = 1; i <= context.n_district; i++) {
      for (int j = 1; j <= 3000; j++) {
        ...
        // MetaDataType
        const customer::value &customer_value =
            *static_cast<customer::value *>(customer_table->search_value(&customer_key));
