...
public:
  ...
  NewOrder(std::size_t coordinator_id, std::size_t partition_id, DatabaseType &db, const ContextType &context,
  ...
  TransactionResult execute(std::size_t worker_id) override
  {
    ...
    this->search_for_read(warehouseTableID, W_ID - 1, storage.warehouse_key, storage.warehouse_value);
    ...
    this->search_for_update(districtTableID, W_ID - 1, storage.district_key, storage.district_value);
    ...
    this->search_for_read(customerTableID, W_ID - 1, storage.customer_key, storage.customer_value);
    ...
    for (int i = 0; i < query.O_OL_CNT; i++) {
      ...
      this->update(stockTableID, OL_SUPPLY_W_ID - 1, storage.stock_keys[i], storage.stock_values[i]);

      if (context.operation_replication) {
        Encoder encoder(this->operation.data);
        encoder << storage.stock_keys[i].S_W_ID << storage.stock_keys[i].S_I_ID << storage.stock_values[i].S_QUANTITY
                << storage.stock_values[i].S_YTD << storage.stock_values[i].S_ORDER_CNT
                << storage.stock_values[i].S_REMOTE_CNT;
      }
  ...
  Task<TransactionResult> execute_coro(std::size_t worker_id) 
  {
    int32_t W_ID = this->partition_id + 1;
    ...
    for (int i = 0; i < query.O_OL_CNT; i++) {
      ...
      if (storage.item_keys[i].I_ID == 0) {
        // abort();
        co_return TransactionResult::ABORT_NORETRY;
      }
    ...
    auto task = this->process_requests_coro(worker_id);
    while (!task.done()) {
    ...
    if (task.get_value())
      co_return TransactionResult::ABORT;
    ...
    co_return TransactionResult::READY_TO_COMMIT;
  }
...
public:
  ...
  Payment(std::size_t coordinator_id, std::size_t partition_id, DatabaseType &db, const ContextType &context,
      RandomType &random, Partitioner &partitioner, Storage &storage)
      : Transaction(coordinator_id, partition_id, partitioner),
        db(db),
        context(context),
  ...
  TransactionResult execute(std::size_t worker_id) override
  {
    ...
    if (context.write_to_w_ytd) {
      ...
      // retrieved and W_YTD,
      storage.warehouse_key = warehouse::key(W_ID);
      this->search_for_update(warehouseTableID, W_ID - 1, storage.warehouse_key, storage.warehouse_value);
    }
    ...
    this->search_for_update(customerTableID, C_W_ID - 1, storage.customer_key, storage.customer_value);
    if (this->process_requests(worker_id))
      return TransactionResult::ABORT;
    ...
    if (this->execution_phase) {
      written = std::sprintf(
          H_DATA, "%s    %s", storage.warehouse_value.W_NAME.c_str(), storage.district_value.D_NAME.c_str());
      H_DATA[written] = 0;
