...
public:
  ...
  MessagePiece(const MessagePiece &messagePiece) : stringPiece(messagePiece.stringPiece) {}

  MessagePiece(const StringPiece &stringPiece) : stringPiece(stringPiece) {}

  uint64_t get_message_type() const { return (get_header() >> MESSAGE_TYPE_OFFSET) & MESSAGE_TYPE_MASK; }

  uint64_t get_message_length() const { return (get_header() >> MESSAGE_LENGTH_OFFSET) & MESSAGE_LENGTH_MASK; }

  uint64_t get_table_id() const { return (get_header() >> TABLE_ID_OFFSET) & TABLE_ID_MASK; }

  uint64_t get_partition_id() const { return (get_header() >> PARTITION_ID_OFFSET) & PARTITION_ID_MASK; }
  ...
  bool operator==(const MessagePiece &that) const { return stringPiece == that.stringPiece; }

  bool operator!=(const MessagePiece &that) const { return stringPiece != that.stringPiece; }

private:
  uint64_t get_header() const { return *reinterpret_cast<const uint64_t *>(stringPiece.data()); }
...
public:
  ...
  static uint64_t construct_message_piece_header(
      uint64_t message_type, uint64_t message_length, std::size_t table_id, std::size_t partition_id)
  {
    ...
    return (message_type << MESSAGE_TYPE_OFFSET) + (message_length << MESSAGE_LENGTH_OFFSET) +
           (table_id << TABLE_ID_OFFSET) + (partition_id << PARTITION_ID_OFFSET);
  }
