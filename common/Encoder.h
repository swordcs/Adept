...
public:
  ...
  StringPiece toStringPiece() { return StringPiece(bytes.data(), bytes.size()); }

  void write_n_bytes(const void *ptr, std::size_t size) { bytes.append(static_cast<const char *>(ptr), size); }
