...
public:
  ...
  FixedString &assign(const std::string &str) { return assign(str, str.length()); }
...
inline std::basic_ostream<C> &operator<<(std::basic_ostream<C> &os, const FixedString<N> &str)
{
  os << str.toString();
  return os;
}
...
namespace std {
template <std::size_t N>
struct hash<aria::FixedString<N>>
{
  std::size_t operator()(const aria::FixedString<N> &k) const { return k.hash_code(); }
};
}  // namespace std
