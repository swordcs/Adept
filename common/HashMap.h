...
public:
  ...
  {
    return apply([&key](const HashMapType &map) { return map.find(key) != map.end(); }, bucket_number(key));
  }
  ...
  {
    return apply_ref([&key](HashMapType &map) -> ValueType & { return map[key]; }, bucket_number(key));
  }

  std::size_t size()
  {
    return fold(0, [](std::size_t totalSize, const HashMapType &map) { return totalSize + map.size(); });
  }
