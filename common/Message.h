...
public:
  ...
  {
  public:
    Iterator(const char *ptr, const char *eof) : eof(eof), messagePiece(get_message_piece(ptr)) {}
    ...
    bool operator==(const Iterator &that) const { return messagePiece == that.messagePiece && eof == that.eof; }
...
public:
  ...
  uint64_t get_source_node_id() { return (get_header_ref() >> SOURCE_NODE_ID_OFFSET) & SOURCE_NODE_ID_MASK; }
  ...
  uint64_t get_dest_node_id() { return (get_header_ref() >> DEST_NODE_ID_OFFSET) & DEST_NODE_ID_MASK; }
  ...
  uint64_t get_worker_id() { return (get_header_ref() >> WORKER_ID_OFFSET) & WORKER_ID_MASK; }

  uint64_t get_message_count() { return (get_header_ref() >> MESSAGE_COUNT_OFFSET) & MESSAGE_COUNT_MASK; }

  uint64_t get_message_length() { return (get_header_ref() >> MESSAGE_LENGTH_OFFSET) & MESSAGE_LENGTH_MASK; }

private:
  void clear_source_node_id() { get_header_ref() &= ~(SOURCE_NODE_ID_MASK << SOURCE_NODE_ID_OFFSET); }

  void clear_dest_node_id() { get_header_ref() &= ~(DEST_NODE_ID_MASK << DEST_NODE_ID_OFFSET); }

  void clear_worker_id() { get_header_ref() &= ~(WORKER_ID_MASK << WORKER_ID_OFFSET); }

  void clear_message_count() { get_header_ref() &= ~(MESSAGE_COUNT_MASK << MESSAGE_COUNT_OFFSET); }

  void clear_message_length() { get_header_ref() &= ~(MESSAGE_LENGTH_MASK << MESSAGE_LENGTH_OFFSET); }
...
private:
  ...
  uint32_t &get_deadbeef_ref() { return *reinterpret_cast<uint32_t *>(&data[0] + sizeof(header_type)); }
...
public:
  static constexpr uint32_t get_prefix_size() { return sizeof(header_type) + sizeof(deadbeef_type); }

  static uint64_t get_message_length(uint64_t v) { return (v >> MESSAGE_LENGTH_OFFSET) & MESSAGE_LENGTH_MASK; }
