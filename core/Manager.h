...
public:
  ...
  virtual void set_txn_generators(std::vector<std::shared_ptr<TxnGenerator>> &txn_generators)
  {
    LOG(FATAL) << "This manager does not implement txn_generators setter.";
  }
  ...
  void set_worker_status(ExecutorStatus status) { worker_status.store(static_cast<uint32_t>(status)); }
  ...
  void send_signal(ExecutorStatus status)
  {
    // only send signal to the coordinator node
    // without setting worker status
    DCHECK(coordinator_id == 0);
  ...
  void push_message(Message *message) override
  {
    ...
    switch (message_type) {
      case ControlMessage::SIGNAL: signal_in_queue.push(message); break;
      case ControlMessage::VECTOR: vector_in_queue.push(message); break;
      case ControlMessage::ACK: ack_in_queue.push(message); break;
      case ControlMessage::STOP: stop_in_queue.push(message); break;
      default: CHECK(false) << "Message type: " << static_cast<uint32_t>(message_type); break;
    }
  }
...
protected:
  ...
  LockfreeQueue<Message *>              ack_in_queue, vector_in_queue, signal_in_queue, stop_in_queue, out_queue;
