...
public:
  IncomingDispatcher(std::size_t id, std::size_t group_id, std::size_t io_thread_num, std::vector<Socket> &sockets,
...
{
public:
  OutgoingDispatcher(std::size_t id, std::size_t group_id, std::size_t io_thread_num, std::vector<Socket> &sockets,
      const std::vector<std::shared_ptr<Worker>> &workers, LockfreeQueue<Message *> &coordinator_queue,
      std::atomic<bool> &stopFlag)
      : id(id),
        group_id(group_id),
  ...
  void sendMessage(Message *message)
  {
    ...
    DCHECK(dest_node_id >= 0 && dest_node_id < sockets.size() && dest_node_id != id);
    DCHECK(message->get_message_length() == message->data.length())
        << message->get_message_length() << " " << message->data.length();

    sockets[dest_node_id].write_n_bytes(message->get_raw_ptr(), message->get_message_length());
