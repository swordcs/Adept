...
public:
  ...
  // Search for a particular key and version.
  virtual std::tuple<MetaDataType *, void *> search(const void *key, uint64_t version = 0) = 0;

  virtual void *search_value(const void *key, uint64_t version = 0) = 0;

  virtual MetaDataType &search_metadata(const void *key, uint64_t version = 0) = 0;

  // Search for the previous version of a particular key.
  virtual std::tuple<MetaDataType *, void *> search_prev(const void *key, uint64_t version) = 0;

  virtual void *search_value_prev(const void *key, uint64_t version) = 0;

  virtual MetaDataType &search_metadata_prev(const void *key, uint64_t version) = 0;
  
  virtual void insert(const void *key, const void *value, uint64_t version = 0) = 0;

  virtual void update(const void *key, const void *value, uint64_t version = 0) = 0;

  // Search for the last version of a particular key.
  virtual MetaDataType &insert_pure_holder(const void *key, uint64_t version = 0) = 0;

  virtual std::tuple<MetaDataType *, void *> search_last(const void *key) = 0;

  virtual void *search_value_last(const void *key) = 0;

  virtual MetaDataType &search_metadata_last(const void *key) = 0;

  virtual MetaDataType &search_metadata_version_last(const void *key, uint64_t version = 0) = 0;

  virtual void update_version_last(const void *key, const void *value, uint64_t version = 0) = 0;

  // Other operations
  virtual void garbage_collect(const void *key) = 0;

  virtual void deserialize_value(const void *key, StringPiece stringPiece, uint64_t version = 0) = 0;
...
public:
  ...
  Table(std::size_t tableID, std::size_t partitionID) : tableID_(tableID), partitionID_(partitionID) {}
  ...
  MetaDataType &search_metadata(const void *key, uint64_t version = 0) override
  {
    const auto &k = *static_cast<const KeyType *>(key);
  ...
  // Search for the previous version of a particular key.
  std::tuple<MetaDataType *, void *> search_prev(const void *key, uint64_t version) override
  {
    const auto &k = *static_cast<const KeyType *>(key);
    auto       &v = map_[k];
    return std::make_tuple(&std::get<0>(v), &std::get<1>(v));
  ...
  MetaDataType &search_metadata_prev(const void *key, uint64_t version) override
  {
    const auto &k = *static_cast<const KeyType *>(key);
    return std::get<0>(map_[k]);
  }
  
  void insert(const void *key, const void *value, uint64_t version = 0) override
  {
    const auto &k = *static_cast<const KeyType *>(key);
  ...
  void update(const void *key, const void *value, uint64_t version = 0) override
  {
    const auto &k    = *static_cast<const KeyType *>(key);
    const auto &v    = *static_cast<const ValueType *>(value);
    auto       &row  = map_[k];
  ...
  MetaDataType &search_metadata_last(const void *key) override
  {
    const auto &k = *static_cast<const KeyType *>(key);
    return std::get<0>(map_[k]);
  }
  ...
  void deserialize_value(const void *key, StringPiece stringPiece, uint64_t version = 0) override
  {
...
public:
  ...
  MVCCTable(std::size_t tableID, std::size_t partitionID) : tableID_(tableID), partitionID_(partitionID) {}
  ...
  std::tuple<MetaDataType *, void *> search(const void *key, uint64_t version = 0) override
  {
    ...
    CHECK(v_ptr != nullptr) << "key with version: " << version << " does not exist.";
    auto &v = *v_ptr;
    return std::make_tuple(&std::get<0>(v), &std::get<1>(v));
  }
  ...
  void update(const void *key, const void *value, uint64_t version = 0) override
  {
    ...
    CHECK(row_ptr != nullptr) << "key with version: " << version << " does not exist.";
    auto &row = *row_ptr;
    std::get<0>(row).store(0);
    std::get<1>(row) = v;
  }
  ...
  MetaDataType &search_metadata_version_last(const void *key, uint64_t version) override {
    const auto &k     = *static_cast<const KeyType *>(key);
    auto       *v_ptr = map_.get_key_version_last(k, version);
    CHECK(v_ptr != nullptr) << "key with version: " << version << " does not exist.";
    auto &v = *v_ptr;
    return std::get<0>(v);
  }

  void update_version_last(const void *key, const void *value, uint64_t version = 0) override
  {
    const auto &k       = *static_cast<const KeyType *>(key);
    const auto &val_new = *static_cast<const ValueType *>(value);
    auto       *row_ptr = map_.get_key_version_last(k, version);
    CHECK(row_ptr != nullptr) << "key does not exist.";
    auto &row        = *row_ptr;
    std::get<1>(row) = val_new;
  }
...
public:
  template <std::size_t N, class KeyType, class ValueType>
  static std::unique_ptr<ITable> create_table(const Context &context, std::size_t tableID, std::size_t partitionID)
  {
    if (context.mvcc) {
      return std::make_unique<MVCCTable<N, KeyType, ValueType>>(tableID, partitionID);
    } else {
